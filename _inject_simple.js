...
// 只处理非地图页面
var files = ['3x3','gear','help','maps','review','strategy','weapons'];

files.forEach(function(n) {
  var fp = 'F:/暗区突围网站/pages/' + n + '.html';
  var c = fs.readFileSync(fp, 'utf-8');

  // 1. 替换 #ffc832 → var(--accent)（只替换 CSS 和 HTML，不替换 JS 字符串里的）
  c = c.split('\n').map(function(l) {
    if (l.indexOf(':root') >= 0 || l.indexOf('--accent:') >= 0 || 
        l.indexOf('--bg-page:') >= 0 || l.indexOf('--card-bg:') >= 0 ||
        l.indexOf('--text-body:') >= 0 || l.indexOf('--btn-') >= 0 ||
        l.indexOf('DEFAULTS') >= 0 || l.indexOf('abi_palette') >= 0 ||
        l.indexOf('applyAll') >= 0) return l;
    l = l.replace(/#ffc832/gi, 'var(--accent)');
    l = l.replace(/#08080e/gi, 'var(--bg-page)');
    ...
    l = l.replace(/color: #888/gi, 'color:var(--text-body)');
    l = l.replace(/color: #ddd/gi, 'color:var(--text-body)');
    l = l.replace(/background:rgba\(255,200,50,/gi, 'background:rgba(var(--accent-rgb),');
    return l;
  }).join('\n');
