...
for fp in files:
    ...
    has_modal = 'openPostModal' in c
    has_no_inline = 'loadComments(p.id' not in c
    print(fp.split('/')[-1] + ': {'+str(ob)+'}={'+str(cb)+'} ('+str(op)+')='+str(cp)+' ' + ok)
    print('  postModal:', has_modal, '| no inline comments:', has_no_inline)
    # Check cachedPosts
    print('  _cachedPosts:', "window._cachedPosts" in c)
