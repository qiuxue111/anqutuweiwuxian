...
for m in maps:
  ...
  if (img) {
    var iw = img.naturalWidth || img.width || 2000;
    var ih = img.naturalHeight || img.height || 2000;
    // transform: translate(panX,panY) scale(s) + transform-origin:0 0
    // 执行顺序：先 translate 再 scale，但 translate 也被 scale 放大！
    // 元素左上角在 transform 后位于 (panX*s, panY*s)
    // 元素中心：panX*s + iw*s/2 = w/2
    // panX = w/(2*s) - iw/2
    scaleM = s;
    panX = w / (2 * s) - iw / 2;
    panY = h / (2 * s) - ih / 2;
