...
for m in maps:
    ...
    # Completely replace the drag-prevention script
    old_script = '''<script>
...
adCard.addEventListener('pointerup',function(e){if(adDrag){e.preventDefault();e.target.closest('a')&&e.target.closest('a').click();}setTimeout(function(){adDrag=false;},50);});
