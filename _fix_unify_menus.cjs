...
// Common: the top section (after <body> and before actual page content)
// In index.html it's: auth script + style + button + userArea + bubbleMenu
// We'll replace the entire <nav> / top area in subpages

// For pages/ subdirectory (maps, gear, strategy, weapons):
// menu links use ../ for index and search, direct for pages
var subMenuPages = '<div id="bubbleMenu"><a href="../index.html">🏠 首页</a><a href="maps.html">🗺 地图选图</a><a href="weapons.html">🔧 改枪</a><a href="strategy.html">💬 聊天</a><a href="gear.html">📖 攻略</a><a href="../search.html">🔍 搜索</a><div class="sep"></div><a href="#" onclick="showUserCenter()">👤 用户中心</a><a href="#" onclick="logout()">🚪 退出登录</a></div>';

// For search.html (root):
var subMenuRoot = '<div id="bubbleMenu"><a href="index.html">🏠 首页</a><a href="pages/maps.html">🗺 地图选图</a><a href="pages/weapons.html">🔧 改枪</a><a href="pages/strategy.html">💬 聊天</a><a href="pages/gear.html">📖 攻略</a><a href="search.html">🔍 搜索</a><div class="sep"></div><a href="#" onclick="showUserCenter()">👤 用户中心</a><a href="#" onclick="logout()">🚪 退出登录</a></div>';

// Common userArea (same as index.html)
...
['maps.html','gear.html','strategy.html','weapons.html'].forEach(function(f){
  ...
  // 1. Remove <nav> ... </nav> entirely (including logo + old userArea + old bubbleMenu)
  c = c.replace(/<nav[^>]*>[\s\S]*?<\/nav>/g, '');
  
  // 2. Remove any leftover old userArea or sideMenu divs
  c = c.replace(/<div\s+id="userArea"[^>]*>[\s\S]*?<\/div>/g, '');
  c = c.replace(/<div\s+id="sideMenu"[^>]*>[\s\S]*?<\/div>/g, '');
  c = c.replace(/<div\s+id="bubbleMenu"[^>]*>[\s\S]*?<\/div>/g, '');
  
  // 3. Insert after <body>: menuBtn + userArea + bubbleMenu
  // But <body> might be followed by an inline auth script ... insert after it or before it
  // Actually, we should insert before any content, right after <body>
  c = c.replace('<body>',
    '<body>\n' + menuBtnHTML + '\n' + userAreaHTML + '\n' + subMenuPages);
  
  // 4. Fix auth: store JSON
  c = replaceCrlf(c,
  ...
  // 5. Add normalizeUser + getUserName
  ...
  // 7. Fix loginGitHub redirect (subpages → index.html)
  c = replaceCrlf(c,
  ...
  // 8. Fix showUserCenter
  c = replaceCrlf(c,
    "alert('用户: '+u+'\\n(更多功能开发中)');",
    "alert('用户: '+getUserName()+'\\n(更多功能开发中)');");
  
  // 9. Fix toggleMenu - should already work with bubbleMenu
  // No need to change, it targets by id which will be bubbleMenu
  ...
  // Check structure
  console.log('  nav='+(c.indexOf('<nav')>=0)+' bubbleMenu='+(c.indexOf('bubbleMenu')>=0)+
    ' menuBtn='+(c.indexOf('menuBtn')>=0)+' userArea='+(c.indexOf('userArea')>=0)+
    ' getUserName='+(c.indexOf('function getUserName')>=0));
});

// ===== Fix search.html (root level) =====
(function fixSearch(){
  ...
  // Auth fixes (search.html uses email)
  c = replaceCrlf(c,
    "localStorage.setItem('abi_user',email);",
    "localStorage.setItem('abi_user',JSON.stringify(payload));");
  c = replaceCrlf(c,
