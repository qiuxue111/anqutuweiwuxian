...
if (c.indexOf(mapNav) < 0 && c.indexOf(target) >= 0) {
  c = c.replace(target, target + mapNav);
  console.log('map nav injected');
} else {
  console.log('map nav already exists or target not found');
}
...
if (c.indexOf('--bg-page') < 0) {
  c = c.replace('<style>', '<style>' + paletteCSS);
  console.log('CSS vars injected');
}

// 3. 替换 body
c = c.replace('body{background:#08080e;color:#ccc;', 'body{background:var(--bg-page);color:var(--text-body);');

// 4. 替换 #ffc832 为 var(--accent)（排除变量定义行）
var lines = c.split('\n');
var newLines = [];
lines.forEach(function(line) {
  if (line.indexOf('--accent:') >= 0 || line.indexOf('--accent-rgb:') >= 0) {
    ...
    newLines.push(line.replace(/#ffc832/g, 'var(--accent)'));
  }
});
c = newLines.join('\n');
...
var paletteBlock = `
<!-- 调色盘入口 -->
<button id="paletteBtn" style="position:fixed;bottom:14px;left:14px;z-index:9999;width:36px;height:36px;border-radius:50%;background:rgba(var(--accent-rgb),0.1);border:1px solid rgba(var(--accent-rgb),0.15);color:var(--accent);font-size:1.1rem;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s;border:none;outline:none;" onclick="togglePalette()" title="自定义配色">🎨</button>
...
c = c.replace('</body>', paletteBlock + '\n</body>');

fs.writeFileSync('F:/暗区突围网站/index.html', c, 'utf-8');
console.log('Done. Size:', c.length);
console.log('farm:', c.includes('map-farm'));
console.log('palette:', c.includes('togglePalette'));
console.log('css vars:', c.includes('--bg-page'));
