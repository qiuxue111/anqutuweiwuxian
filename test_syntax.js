...
function addPin(tid){
  ...
  if(!pp)return;
  supabase("pending_pins","POST",{x:pp.x,y:pp.y,name:TYPE[tid].name,type:tid,ic:TYPE[tid].ic,note:"",images:[],comments:[],admin_passed:false,votes:0,voters:[]});
...
function initAuth(){
  ...
  if(token&&refresh){
    ...
    localStorage.setItem("abi_refresh",refresh);
    window.location.hash="";
  }
  var saved=localStorage.getItem("abi_token");
  if(saved){
    supabase("user","GET",null).then(function(u){
      if(u&&u.id){currentUser=u;updateAuthUI();}
