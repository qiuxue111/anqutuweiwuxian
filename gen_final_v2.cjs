...
// Insert video container + + button before each map-detail closing </div>
const mapInfo = [
  ['beishan', '北山'],
  ['valley', '山谷'],
  ['armory', '军械库'],
...
mapInfo.forEach(([id, name]) => {
  const closeDiv = `  </div>\n\n    <!-- ${name}`;
  // For farm, comment is just '<!-- 农场' not '<!-- 农场 (ABI 专属)'
  const insertBefore = c.indexOf(closeDiv);
  
  if (insertBefore >= 0) {
    // Find the position just after the button's </div> and before the map-detail closing
    const mapDetailClose = c.lastIndexOf('    </div>', insertBefore);
    // This is the closing </div> of map-detail. Insert before it.
    const insertHtml = `      <div class="map-videos" id="videos-${id}"></div>
      <div style="margin-top:0.5rem;">
        <button class="add-video-btn" onclick="openVideoForm('${id}','${name}')" title="添加B站视频">+</button>
      </div>
    `;
    c = c.substring(0, mapDetailClose) + insertHtml + c.substring(mapDetailClose);
    console.log(`${id}: inserted at ${mapDetailClose}`);
  } else {
    console.log(`${id}: could not find close "${closeDiv}"`);
  }
});
...
c = c.replace('id="farm"', 'id="farm"');  // noop check
const footerIdx = c.lastIndexOf('<footer>');
const videoPanel = `<div id="videoPanel" style="display:none;margin-top:2rem;padding:1.5rem;background:#12121a;border:1px solid #1e1e2a;border-radius:12px;">
...
`;
c = c.substring(0, footerIdx) + videoPanel + c.substring(footerIdx);
...
<script>
  ...
  function initAuth() {
    ...
    if (u) currentUser = u;
    if (currentToken) {
      var btn = document.getElementById('videoAdminBtn');
  ...
  function supa(method, table, body, q) {
    ...
    return fetch(url, {
      method: method,
      headers: { 'Authorization': 'Bearer ' + bearer, 'apiKey': anonKey, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
  ...
  async function renderVideoAdmin() {
    try {
      ...
      var grouped = {}; Object.keys(mapNames).forEach(function(k) { grouped[k] = []; });
      videos.forEach(function(v) { var mk = v.map_name || 'farm'; if (grouped[mk]) grouped[mk].push(v); });
      document.getElementById('videoMapBtns').innerHTML = Object.entries(mapNames).map(function(e) {
        return '<button onclick="openVideoForm(\\'' + e[0] + '\\',\\'' + e[1] + '\\')" style="padding:0.3rem 0.8rem;background:#1a1a22;color:#ffc832;border:1px solid #333;border-radius:6px;cursor:pointer;">+' + e[1] + '</button>';
      }).join('');
...
['beishan','valley','armory','farm','airport','tvstation'].forEach(id => {
  console.log('  ' + id + ': videos=' + c.includes('id="videos-' + id + '"') + ', +btn=' + c.includes("openVideoForm('" + id));
});
